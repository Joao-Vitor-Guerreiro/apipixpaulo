...
export class webhookAllowPaymentsController {
  static async main(req: Request, res: Response) {
    try {
      const { data, id, status, payment_id, transaction_id } = req.body;

      // Allow Payments pode enviar o ID de diferentes formas
      ...
      console.log(
        `====> Webhook Allow Payments Recebido! Status: ${paymentStatus} | PaymentId: ${paymentId}`
      );
      ...
      if (isApproved) {
        try {
          ...
          if (utmResponse.ok) {
            ...
            console.log("✅ Resposta da UTMIFY ao WEBHOOK Allow Payments: ", utmResponseJson);
          } else {
      ...
      console.error("❌ Erro no webhook Allow Payments:", error);
      res.status(500).json({ 
